...
def init_match_data(sid,mid,upload=False):
	...
	if os.path.isfile(pickle_file):
		unpicked 	= pickle.load(open(pickle_file,"rb"))
		hero_df 	= unpicked[0]
		actions_df 	= unpicked[1]
		sdf 		= unpicked[2]
		hp_df 		= unpicked[3]
		m_score 	= unpicked[4]
		m_spikes 	= unpicked[5]
		m_attacks 	= unpicked[6]
		t_spikes 	= unpicked[7]
		t_kills 	= unpicked[8]
		t_dmg 		= unpicked[9]
		ht_mean 	= unpicked[10]
	...
	# otherwise if no cache file already, create one
	else:
...
def main(con):
	...
	hero_df,actions_df,sdf,hp_df,m_score,m_spikes,m_attacks,t_spikes,t_kills,t_dmg,ht_mean,ht_med,ht_var = init_match_data(ss.sid,ss.mid)
